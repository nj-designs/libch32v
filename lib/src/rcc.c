...
static volatile uint32_t *const peripheral_reset_reg_look_up[] = {
    &rcc.ahbrstr, &rcc.apb1prstr, &rcc.apb2prstr};
...
static const struct RCCCfgValues cfg_table[] = {
    {.hse_freq = 8'000'000,
     ...
     .cfgr0 = RCC_CFGR0_MCO_NO_CLK | RCC_CFGR0_USBPRE_DIV_3 |
              ...
              RCC_CFGR0_PPRE2_DIV_1 | RCC_CFGR0_PPRE1_DIV_4},
};
...
void rcc_init(void) {
  ...
  rcc.cfgr0 &=
      (uint32_t)~(RCC_CFGR0_MCO_MASK | RCC_CFGR0_ADCPRE_MASK |
...
void rcc_reset_peripherial(RCCPeripheralId id) {
  volatile uint32_t *clk_enable_reg =
      peripheral_reset_reg_look_up[(uint32_t)id >> 16];
