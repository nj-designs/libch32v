...
void i2c_cfg(enum I2CId id, const struct I2CCfgValues* cfg) {
  ...
  if (reg != nullptr) {
    ...
    freq = (uint16_t)(clks->pclk1_freq / 1'000'000);
    ...
    if (cfg->bus_speed <= 100'000) {
      result = (uint16_t)(clks->pclk1_freq / (cfg->bus_speed << 1));
      if (result < 4) {
        result = 4;
      }
      tmpreg |= result;
      ...
      if (cfg->duty_cycle == I2C_DUTY_CYCLE_2) {
        result = (uint16_t)(clks->pclk1_freq / (cfg->bus_speed * 3));
      } else {
        result = (uint16_t)(clks->pclk1_freq / (cfg->bus_speed * 25));
